...
namespace AD.admin
{
    public partial class ad_team : BasePage
    {
        ...
        private void bind()
        {
            ...
            if (key != "")
                where += " and ((id like '%" + key + "%') or (team_name like '%" + key + "%') or (post_job ='" + key + "'))";
            List<T_ad_team> list = ad_main.admin_select_team("t_ad_team", "id", 10, Pager1.CurrentIndex, " id desc", where, out pagecount);
        ...
        protected void rptUser_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            {
                string path = ad_main.admin_delete_img(id, "post_img", "t_ad_team");
                if (path != "")
                {
                    File.Delete(Server.MapPath(path));
                    if (ad_main.admin_delete_id(id, "t_ad_team") > 0)
                    {
